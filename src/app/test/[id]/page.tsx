...
export default function TestEnvironment() {
  ...
  const topicId = decodeURIComponent(params.id as string);
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (data.success && data.data) {
          // Filter by topic from URL
          const allQs = data.data;
          const filteredQs = allQs.filter((q: any) => {
             const qTopic = q.topic || q.subject;
             return qTopic === topicId;
          });
          // Limit to max 150 per test to not overwhelm browser
          setQuestions(filteredQs.slice(0, 150));
        }
      } catch (err) {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 flex flex-col min-h-[calc(100vh-4rem)]">
      {/* Test Header */}
      <div className="flex justify-between items-center mb-6 bg-card p-4 rounded-xl shadow-sm border">
        <div>
          <h1 className="text-xl font-bold">Child Development - Topic Test</h1>
